    ...
    var bridge = new Bridge({
        ...
        controller: {
            ...
            thirdPartyLookup: {
                ...
                parseLocation: this.parseThirdPartyLocation.bind(this),
                getUser: this.getThirdPartyUser.bind(this),
                parseUser: this.parseThirdPartyUser.bind(this),
            },
        }
    });
...
function isValidGitterRoomname(room) {
    ...
    if(!isValidGitterId(parts[0]) ||
            (parts.length > 1 && !isValidGitterId(parts[1]))) return false;
...
MatrixGitterBridge.prototype.parseThirdPartyLocation = function(alias) {
    var result = this.alias_template.matchId(alias);
    if (!result) {
        return Promise.resolve([]);
    }

    var roomname = result.ROOM.toLowerCase().replace(/=2F/i, "/");
...
MatrixGitterBridge.prototype.getThirdPartyUser = function(protocol, fields) {
    ...
    var username = fields.username.replace(/^@/, "").toLowerCase();

    if (!isValidGitterId(username)) {
        return Promise.reject({code: 400, err: "Invalid username"});
    }

    var userid = this.mxidFromGitterName(username);
...
MatrixGitterBridge.prototype.parseThirdPartyUser = function(userid) {
    var result = this.username_template.matchId(userid);
    if (!result) {
        return Promise.resolve([]);
    }

    var username = result.USER.toLowerCase();
